...
def process_scene(scene: dict, tmp_dir: Path, voice: str) -> float:
    ...
    with TemporaryDirectory(prefix="expertlens-vo-") as tmp:
        ...
        for scene in SCENES:
            ...
            if m4a_path.exists() and not args.force:
                ...
                duration = process_scene(scene, tmp_dir, args.voice)
                print(f"{duration:.2f}s  ({int(duration * FPS)} frames)")
