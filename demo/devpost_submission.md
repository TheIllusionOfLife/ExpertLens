...
- **30+ minute sessions** via SlidingWindow `contextWindowCompression` — no 2-minute cutoffs, tested with continuous screen sharing
- **Cross-session memory** with <1 second of added session-start latency (1s Firestore timeout, graceful fallback if slow)
- **~2 second reconnection** on WebSocket rotation — users see "Reconnecting…" briefly and continue with full context via `sessionResumption`
- **2–4 minutes per coach** to build a complete six-section knowledge base using `gemini-3-flash-preview` with Google Search grounding
- **<1 second memory injection overhead** — session summaries loaded from Firestore and injected into system instruction at session start
